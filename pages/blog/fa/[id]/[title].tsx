...
import { filterArticleSource } from '@/components/blog/helpers/filter-article-source'
import { formatBlogPostUrl } from '@/components/blog/helpers/format-blog-post-url'
import { blogPostRedirect } from '@/components/blog/helpers/redirect-blog-post'
	...
	return (
		<>
			...
			<Meta
				...
				type='article'
			/>
			<MotionBox className='progress-bar' style={{ scaleX: scrollYProgress }} />
			...
			<MotionBox
				...
				<CallToAction color='gray.900' bg='yellowish.100'>
					<Box
						...
						mt='-5px'
						display='flex'>
						این مقاله ابتدا در ویرگول نوشته شده
					</Box>
					{data.readFrom.map((item) => (
						<AnimatedButton
							key={item.id}
...
export async function getStaticProps({ params }) {
	...
	const article = {
		data: filterArticleSource(params.id, PersianPublishedArticlesData),
	}
